...
**问题**: 函数认知复杂度 29/25，168 行混合多个职责

**改进方案**:
- 创建 MasterContext 结构体封装运行时状态
- 创建 FileProcessingHandler 处理文件逻辑
- 简化主循环为清晰的 tokio::select!
...
**问题**: 函数认知复杂度 26/25，94 行嵌套 if-let

**改进方案**:
- 创建 AdsPowerSession 结构体
- 拆分为 6 个小函数 (acquire_thread, prepare_session, build_command, execute, cleanup)
...
- 定义 AppResult<T> 和 UnitResult

### P3: 拆分 EmailMonitor IMAP 会话管理 ✅
**改进方案**:
- 拆分为 create_imap_session, search_unread_emails, process_email_batch
