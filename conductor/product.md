...
2.  **Get Track Description:**
    *   **If `开始准备超级扫描仪` contains a description:** Use the content of `开始准备超级扫描仪`.
    *   **If `开始准备超级扫描仪` is empty:** Ask the user:
...
## 产品愿景
开发一款名为 "auto-scanner" 的命令行工具，旨在帮助个人用户自动化批量验证 Facebook 账号的登录状态。该工具将极大简化手动验证账号的过程，提高效率，并提供清晰的结果记录。

## 核心功能
1.  **批量账号导入**: 支持从 CSV 文件中读取 Facebook 账号和密码信息。
2.  **自动登录验证**: 模拟浏览器行为，自动尝试登录 Facebook。
3.  **状态检测与标记**:
    *   **登录成功**: 识别并记录成功登录的状态。
    *   **需验证/2FA**: 识别登录过程中是否遇到验证码或双重验证（2FA）请求，并将其标记存档，不进行后续操作。
    *   **登录失败**: 记录密码错误或其他导致登录失败的原因。
4.  **结果存储**: 将所有账号的验证结果（成功、需验证、失败等）存储到本地 SQLite 数据库中，方便后续查询和管理。

## 目标用户
*   有批量管理或验证 Facebook 账号需求的普通个人用户。

## 用户体验
*   **交互方式**: 简洁的命令行界面 (CLI)。
*   **操作流程**: 用户在终端运行程序，指定输入的 CSV 文件路径，程序自动运行并在终端显示实时进度日志。运行结束后，用户可查询 SQLite 数据库获取详细结果。
