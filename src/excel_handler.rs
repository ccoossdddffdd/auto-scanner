...
pub fn read_accounts_from_excel<P: AsRef<Path>>(
    path: P,
    ...
    let username_idx = headers.iter().position(|h| h.to_lowercase().contains("username") || h.to_lowercase().contains("email") || h.to_lowercase().contains("用户")).context("Username column not found")?;
    let password_idx = headers.iter().position(|h| h.to_lowercase().contains("password") || h.to_lowercase().contains("pass") || h.to_lowercase().contains("密码")).context("Password column not found")?;
