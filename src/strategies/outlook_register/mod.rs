...
impl OutlookRegisterStrategy {
    ...
    async fn type_with_delay(&self, adapter: &dyn BrowserAdapter, selector: &str, text: &str) -> Result<()> {
...
impl BaseStrategy for OutlookRegisterStrategy {
    async fn run(
        ...
        info!("Generated User Info: {} {}, Email: {}", user_info.first_name, user_info.last_name, full_email);
        ...
        info!("Filling email...");
        adapter.wait_for_element("input[name=\"MemberName\"]").await.context("Waiting for email input")?;
        self.type_with_delay(adapter, "input[name=\"MemberName\"]", &full_email).await?;
        
        adapter.click("#iSignupAction").await.context("Clicking Next after email")?;
        self.random_sleep().await;

        // 3. Fill Password
        // Common selectors: input[type="password"], #PasswordInput
        info!("Filling password...");
        adapter.wait_for_element("input[name=\"PasswordInput\"]").await.context("Waiting for password input")?;
        self.type_with_delay(adapter, "input[name=\"PasswordInput\"]", &user_info.password).await?;
        
        adapter.click("#iSignupAction").await.context("Clicking Next after password")?;
        self.random_sleep().await;
        ...
        info!("Filling name...");
        adapter.wait_for_element("input[name=\"FirstName\"]").await.context("Waiting for first name input")?;
        self.type_with_delay(adapter, "input[name=\"FirstName\"]", &user_info.first_name).await?;
        
        adapter.wait_for_element("input[name=\"LastName\"]").await.context("Waiting for last name input")?;
        self.type_with_delay(adapter, "input[name=\"LastName\"]", &user_info.last_name).await?;
        
        adapter.click("#iSignupAction").await.context("Clicking Next after name")?;
        self.random_sleep().await;
        ...
        data.insert("email".to_string(), serde_json::Value::String(full_email.clone()));
        data.insert("password".to_string(), serde_json::Value::String(user_info.password.clone()));
        data.insert("first_name".to_string(), serde_json::Value::String(user_info.first_name));
        data.insert("last_name".to_string(), serde_json::Value::String(user_info.last_name));
        data.insert("birth_year".to_string(), serde_json::Value::Number(serde_json::Number::from(user_info.birth_year)));
